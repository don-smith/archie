...
| `architecture-docs` | Evidence-backed architecture documentation | Claims, authored pages, ordered page map, preview, and handoff | A maintainer approves claims. HTML Design owns the final site. | Requires the target's pinned Architecture Docs package and repository evidence. |
...
| `structural-inspection` | A bounded, layer-by-layer module inspection | Layered advisory findings and phased options | The developer triages findings before follow-up. Findings are not an architecture pass. | Starts through Archie. The current host may not provide an owner. |
