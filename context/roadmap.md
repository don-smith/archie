...
- **Public release `0.3.0` (`765e2b4`, `eff137e`, `94e7572`, `511ddff`, `8df72bc`; tag `v0.3.0`).** The VRS intent tree became the maintainer source of truth (nine obsolete `docs/archie/` sources absorbed, the two deployed reference sources kept), the lockstep product identity was aligned to `0.3.0` with current-facing private-trial wording corrected, release-record schema v3 and the legacy authorization wire value remain unchanged with old and new receipts selectable, candidate evidence was retained, and the annotated `v0.3.0` tag was created at `8df72bc` and pushed.
