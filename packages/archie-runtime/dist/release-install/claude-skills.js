...
const claudeDirectory = (targetDirectory) => join(resolve(targetDirectory), ".claude");
const skillsDirectory = (targetDirectory) => join(claudeDirectory(targetDirectory), "skills");
const linkPath = (targetDirectory, skill) => join(skillsDirectory(targetDirectory), skill);
const linkTo = (skill) => `${LINK_PREFIX}${skill}`;
 ...
 */
function occupancy(targetDirectory, skill) {
    const path = linkPath(targetDirectory, skill);
    const link = readLink(path);
    if (link !== undefined)
        return link.startsWith(LINK_PREFIX) ? "ours" : "occupied";
 ...
 */
export function claudeBridgePaths(targetDirectory, skills, previousSkills = []) {
    const target = resolve(targetDirectory);
    if (!existsSync(claudeDirectory(target)))
        return [];
 ...
 */
export function linkClaudeSkills(targetDirectory, skills, previousSkills = []) {
    const target = resolve(targetDirectory);
    if (!existsSync(claudeDirectory(target)))
        return { status: "not-applied", linked: [], removed: [], occupied: [] };
    mkdirSync(skillsDirectory(target), { recursive: true });
    const linked = [], occupied = [];
    for (const skill of skills) {
        if (occupancy(target, skill) === "occupied") {
            occupied.push(skill);
            continue;
        }
        const path = linkPath(target, skill);
        if (readLink(path) !== linkTo(skill)) {
            rmSync(path, { force: true });
            symlinkSync(linkTo(skill), path);
        }
        linked.push(skill);
    }
    const removed = [];
    for (const skill of previousSkills) {
        if (skills.includes(skill) || occupancy(target, skill) !== "ours")
            continue;
 ...
 */
export function verifyClaudeSkillLinks(targetDirectory, skills) {
    const target = resolve(targetDirectory);
    if (!existsSync(claudeDirectory(target)))
        return "not-applied";
    if (!existsSync(skillsDirectory(target)))
        return "passed";
    ...
    for (const skill of skills) {
        ...
        if (link === undefined || !link.startsWith(LINK_PREFIX))
            continue;
        if (link !== linkTo(skill))
            throw new Error(`Claude Code skill link .claude/skills/${skill} points at ${link} rather than the skill of the same name; re-run the Archie install to reconcile it`);
        if (!existsSync(linkPath(target, skill)))
            throw new Error(`Claude Code skill link .claude/skills/${skill} is dangling: ${link} does not exist; re-run the Archie install to reconcile it`);
    }
    return "passed";
}
