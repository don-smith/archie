...
test("the two-way-runtime false-positive assertion distinguishes negation from a defect claim", async () => {
  const config = JSON.parse(await readFile(new URL("../evaluation/evals.json", import.meta.url), "utf8"));
...
test("canonical controlled fixtures preserve the frozen source hashes", async () => {
  const root = path.resolve(import.meta.dirname, "..");
  const config = JSON.parse(await readFile(path.join(root, "evaluation/evals.json"), "utf8"));
  for (const caseDefinition of config.cases) {
    const canonical = await hashTree(path.join(root, "evaluation", caseDefinition.fixture));
    const frozen = await hashTree(path.join(root, "test/fixtures/evaluations", caseDefinition.id));
    assert.deepEqual(canonical, frozen, caseDefinition.id);
  }
});
