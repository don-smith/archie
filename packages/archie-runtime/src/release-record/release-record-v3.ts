...
export const RELEASE_RECORD_SCHEMA_VERSION = 3 as const;
export const RELEASE_RECORD_FILE = "release-record-v3.json" as const;
export const BUNDLE_INPUT_FORMAT = "archie-private-bundle-input-v3" as const;
...
export const RELEASE_ARTIFACT_PACKAGES = ["@archie/runtime", "@archie/conformance"] as const;
...
export interface FinalizeReleaseResult { record: ReleaseRecord; recordPath: string; receiptPath: string; recordSha256: string; }
export interface SelectedRelease {
  bundleDirectory: string;
  record: ReleaseRecord;
  recordBytes: string;
  recordSha256: string;
  artifacts: { record: ReleaseArtifact; tarballPath: string; tarballName: string }[];
  npmLockBytes: string;
}
...
function stringMap(value: unknown, label: string): Record<string, string> {
  ...
  for (const key of Object.keys(source).sort()) result[key] = text(source[key], `${label}.${key}`);
  return result;
}
function within(root: string, file: string): string {
  const path = resolve(root, file);
  if (relative(root, path).startsWith("..")) throw new Error(`bundle path escapes its directory: ${file}`);
  if (!existsSync(path) || !statSync(path).isFile()) throw new Error(`bundle artifact is missing: ${file}`);
  return path;
}
...
function bundleInput(root: string): BundleInput {
  ...
  if (value.format !== BUNDLE_INPUT_FORMAT) throw new Error("bundle input has an unsupported format");
  if (!Array.isArray(value.artifacts) || value.artifacts.length !== RELEASE_ARTIFACT_PACKAGES.length) throw new Error("bundle input must contain exactly the runtime and conformance artifacts");
...
function field(source: string, name: string, label: string): string {
...
function skillSubset(source: string, header: string, label: string): string[] {
...
function apmEvidence(root: string, input: BundleInput["apm"]): ReleaseRecord["apm"] {
  ...
  if (field(manifest, "git", "APM manifest") !== input.locator || field(manifest, "ref", "APM manifest") !== input.ref) throw new Error("APM manifest differs from bundle input");
...
function artifactEvidence(root: string, input: BundleArtifact): ReleaseArtifact {
  if (!input.locator.startsWith("file:npm/") || input.locator.includes("..")) throw new Error("artifact locators must be target-owned local tarballs");
  const tarball = readFileSync(within(root, input.tarball));
  const inspected = inspectNpmTarball(tarball);
  const manifest = inspected.manifest;
  if (manifest.name !== input.package || manifest.version !== input.version) throw new Error("npm tarball package identity differs from bundle input");
  const requiredPlatformPayload = normalizeRequiredPlatformPayload(input.requiredPlatformPayload);
  if (!inspected.files.has(`package/${requiredPlatformPayload}`)) throw new Error(`npm tarball omits required platform payload: ${requiredPlatformPayload}`);
  ...
  if (packageLock.version !== input.version || packageLock.resolved !== input.locator || packageLock.integrity !== sha512Integrity(tarball)) throw new Error("npm lock evidence does not match finalized tarball bytes");
  return {
...
function analyzer(): ReleaseRecord["analyzerCompatibility"] {
...
function validateArtifact(value: unknown, label: string): asserts value is ReleaseArtifact {
  ...
  for (const key of ["package", "version", "locator", "lockIntegrity", "requiredPlatformPayload"]) text(artifact[key], `${label}.${key}`);
  const locator = String(artifact.locator);
  if (!locator.startsWith("file:npm/") || locator.includes("..") || !String(artifact.lockIntegrity).startsWith("sha512-") || !isSha256(artifact.tarballSha256) || normalizeRequiredPlatformPayload(String(artifact.requiredPlatformPayload)) !== artifact.requiredPlatformPayload) throw new Error(`${label} evidence is malformed`);
...
function validateRecord(record: ReleaseRecord): void {
  ...
  if (record.schemaVersion !== RELEASE_RECORD_SCHEMA_VERSION || record.product !== "archie" || !record.version || !/^[a-f0-9]{40}$/i.test(String(record.sourceCommit))) throw new Error("unsupported or malformed release record");
...
export function parseReleaseRecord(bytes: string): ReleaseRecord {
  const value = object(parseJson(bytes, "release record"), "release record");
  if (value.schemaVersion !== RELEASE_RECORD_SCHEMA_VERSION) throw new Error(`unsupported release record schema version: ${String(value.schemaVersion)}; Archie now reads only release record v${RELEASE_RECORD_SCHEMA_VERSION}`);
  validateRecord(value as unknown as ReleaseRecord);
  if (serializeReleaseRecord(value as unknown as ReleaseRecord) !== bytes) throw new Error("release record bytes are not canonical");
...
export function validateBundleLayout(bundleDirectory: string): void {
  ...
  if (!existsSync(join(root, "bundle.json")) || !statSync(join(root, "bundle.json")).isFile()) throw new Error("bundle layout is missing bundle.json");
  for (const required of ["npm", "apm"]) if (!existsSync(join(root, required)) || !statSync(join(root, required)).isDirectory()) throw new Error(`bundle layout is missing ${required}`);
}
...
export function finalizeRelease(request: FinalizeReleaseRequest): FinalizeReleaseResult {
  ...
  if (bundledLock !== generatedNpm.lock) throw new Error("bundle npm lock differs from the exact generated projection");
...
export function selectLocalRelease(directory: string): SelectedRelease {
  ...
  const artifacts = input.artifacts.map((inputArtifact, index) => {
    ...
    if (inputArtifact.package !== artifact.package || inputArtifact.version !== artifact.version || inputArtifact.locator !== artifact.locator || artifact.locator !== `file:npm/${tarballName}` || packageLock.integrity !== artifact.lockIntegrity || packageLock.resolved !== artifact.locator || sha512Integrity(tarball) !== artifact.lockIntegrity || sha256(tarball) !== artifact.tarballSha256) throw new Error("selected bundle artifact evidence differs from its finalized record");
  ...
  const generatedNpm = npmProjection(record);
  if (npmLockBytes !== generatedNpm.lock) throw new Error("selected bundle npm lock differs from the exact generated projection");
  validateNpmProjection({ manifest: generatedNpm.manifest, lock: npmLockBytes }, record);
  if (canonicalize(apmEvidence(root, input.apm)) !== canonicalize(record.apm)) throw new Error("selected bundle APM evidence differs from its finalized record");
