...
| Architecture Review | Proactive review of one bounded module | Triaged findings and phased polish plan | Developer triage first | Findings, not an architecture pass |
| HTML Design | Self-contained HTML presentation | Checked site, packet, or document | Presentation only; content owners decide content | Artifact profile validity |
...
MyFlow lifecycle behavior, general development skills, named-agent dependencies, `codebase-locator`, and `codebase-analyzer` are not shipped or required. Architecture Review ships as an Archie-owned skill adapted from MyFlow: it uses host-neutral developer checkpoints and may run its analysis serially. Codebase Design is not a shipped skill; its deep-module vocabulary ships as a shared reference used by Assessment and Architecture Review. Repository-local checks remain target-owned commands with their original exits and evidence meanings.

The source-import manifest records exact sibling revisions for current imports and completed migrations. Architecture Docs was migrated from the former `c4archviewer` proof at `f2b7c5f1353c5e76fe8739a657632edafc66580f`, with later managed-site documentation and architecture status consumer changes ported at `81a008e629de8ff28b5db01a436c4425827feff5`; its generated example site was deliberately excluded. Assessment and Conformance now have Archie-owned canonical workspaces backed by exact per-path migration inventories. The former sibling repositories for Architecture Docs, Assessment, and Conformance were retired on 2026-09-17; the manifest and migration inventories remain historical records. Signing, trusted distribution, key material, public release trust, and publication are deferred.
