...
| ID | Item | Kind |
|---|---|---|
| N0 | The monorepo has no Git remote, so its source exists only on this machine. The private GitHub repository `don-smith/archie` holds only the published APM skill context, with its own two-commit history. Decide where the monorepo itself should live: for example a second private GitHub repository added as this checkout's remote. | decision (developer) |
