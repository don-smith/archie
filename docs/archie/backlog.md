...
| ID | Item | Kind |
|---|---|---|
| N0 | **Decided on 2026-09-18: a scripted install that wraps `archie bootstrap`.** Installation is `git clone --depth 1 git@github.com:don-smith/archie.git /tmp/archie-install && /tmp/archie-install/install.sh`, run from inside the target repository. `curl \| bash` is not an option: `raw.githubusercontent.com` refuses a private repository without a token, whereas the clone reuses the SSH access the developer already has, and it lets them read the script before running it. The script calls `archie bootstrap` rather than copying files, because `dispatch-runtime.mjs` hard-fails without `.archie/release/release-record-v3.json`, and because the record carries `archie verify`, the install journal and compensation, tarball integrity, and per-file skill content hashes. APM stays: the script hides it from the developer, so it is an implementation detail rather than a user-facing dependency, removable later if it gets in the way. Groundwork already verified: the monorepo is `main` on GitHub with the `v0.1.0-private.0` and `v0.1.0-private.1` tags still resolving (the `v0.1.0-private.1` lock is unchanged), and `apm install don-smith/archie/packages/archie-context#<ref>` installs all eight skills byte-identically from the subfolder, locked with `virtual_path: packages/archie-context`, `is_virtual: true`, `package_type: apm_package`. Implementation is N0a. | decided |
| N0a | **Build `install.sh` and `uninstall.sh` in the repository root**, heavily commented so a developer can audit them before running. Install: check `node >=24 <25` and fail with a clear message; detect an existing install and offer upgrade; detect a release record v1 or v2 pin and name exactly what to remove (this subsumes N2); run `archie bootstrap`; install Playwright browsers (P7); finish by telling the developer to invoke the `archie` skill in their coding agent, so onboarding leads them to the first artifacts. Uninstall: read the install journal rather than re-deriving paths; remove only the eight recorded skill trees, never `rm -rf` a whole skills directory that may hold skills Archie does not own; **keep `.archie/assessments/`** and say so in the output, since it holds the developer's own work product. Both scripts must be re-runnable. Tagging monorepo commits for releases, release record v3's APM evidence and projection (they expect a root package named `archie-context` with `git:` and `skills:` entries and would need the virtual path), the bundle fixture, the published-context test, and `docs/archie/private-release-bundle.md` all follow from N0 keeping APM. Note that the existing tags point at context-only commits, not monorepo history. | implementation |
