...
const packages = [
  ...
  ["@archie/assessment", "assessment"], ["@archie/conformance", "conformance"], ["@archie/html-design", "html-design"], ["@archie/architecture-review", "architecture-review"],
...
for (const [name, directory] of packages) {
  ...
  const unsafeAllowlist = !Array.isArray(manifest.files) || manifest.files.some((file) => /(^|\/)(test|\.myflow|skills)(\/|$)/.test(file) && !((name === "@archie/context" && file === ".apm/skills") || (["@archie/architecture-docs", "@archie/assessment", "@archie/conformance", "@archie/html-design", "@archie/architecture-review"].includes(name) && file === "skills")));
...
for (const [name] of packages) {
  ...
  if (name === "@archie/architecture-review") {
    for (const required of ["skills/architecture-review/SKILL.md", "skills/architecture-review/templates/architecture-review.md", "skills/architecture-review/references/deep-module-vocabulary.md"]) {
      if (!packed.includes(required)) throw new Error(`Architecture Review package omits required skill asset: ${required}`);
    }
  }
