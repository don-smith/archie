...
function safePath(value, name) { const result = string(value, name, MAX_PATH); if (result.startsWith("/") || result.includes("\\") || /^[A-Za-z]:/.test(result) || /^[A-Za-z][A-Za-z0-9+.-]*:/.test(result) || result.split("/").some((part) => !part || part === "." || part === "..")) fail(`${name} is unsafe`); return result; }
...
function rejectUnknown(value, allowed, name) {
  for (const key of Object.keys(value)) if (!allowed.has(key)) fail(`${name}.${key} is not a supported field`);
}
function normalizeCounts(value, name) {
  if (value === undefined) return undefined;
  if (!Array.isArray(value) || value.length > 100) fail(`${name} is invalid`);
  const result = value.map((entry, index) => { const item = object(entry, `${name}[${index}]`); rejectUnknown(item, new Set(["id", "value"]), `${name}[${index}]`); const itemId = id(item.id, `${name}[${index}].id`); if (!Number.isSafeInteger(item.value) || item.value < 0 || item.value > 1_000_000_000) fail(`${name}[${index}].value is invalid`); return { id: itemId, value: item.value }; });
  if (new Set(result.map((entry) => entry.id)).size !== result.length) fail(`${name} must have unique IDs`);
...
function normalizeExecution(value, name) {
  ...
  if (item.state === "not-run") { rejectUnknown(item, new Set(["state", "reason"]), name); return { state: item.state, reason: string(item.reason, `${name}.reason`) }; }
  rejectUnknown(item, item.state === "completed" ? new Set(["state", "startedAt", "finishedAt", "exitCode"]) : new Set(["state", "startedAt", "finishedAt", "reason"]), name);
...
function normalizeResult(value, name) {
  const item = object(value, name); if (item.state === "unknown") { rejectUnknown(item, new Set(["state", "reason"]), name); return { state: "unknown", reason: string(item.reason, `${name}.reason`) }; }
  if (item.state !== "reported") fail(`${name}.state is unsupported`);
  rejectUnknown(item, new Set(["state", "code", "label", "summary", "counts", "findingIds"]), name);
...
function normalizeEvidence(value, name) {
  ...
  if (item.state === "missing") { rejectUnknown(item, new Set(["state", "reason"]), name); return { state: "missing", reason: string(item.reason, `${name}.reason`) }; }
  if (item.state !== "present" || !["current", "retained"].includes(item.source)) fail(`${name} is unsupported`);
  rejectUnknown(item, new Set(["state", "source", "path", "sha256", "observedAt", "observedRevision"]), name);
...
function normalizeFreshness(value, name) {
  const item = object(value, name); rejectUnknown(item, new Set(["state", "reasons"]), name); if (!states.has(item.state)) fail(`${name}.state is unsupported`);
...
async function safeEvidenceRoot(rootDirectory, relative) {
  const root = await realpath(path.resolve(rootDirectory));
  const target = await realpath(path.resolve(rootDirectory, relative));
  if (target !== root && !target.startsWith(`${root}${path.sep}`)) fail("evidence path escapes the architecture-docs root", "$.checks.evidence.path");
  return target;
}
...
export async function loadArchitectureStatus(config) {
  ...
  for (const [index, check] of snapshot.checks.entries()) if (check.evidence.state === "present") {
    let evidenceFile; let bytes;
    try { evidenceFile = await safeEvidenceRoot(config.paths.rootDirectory, check.evidence.path); bytes = await readFile(evidenceFile); } catch (error) { if (error?.code === "ARCHITECTURE_STATUS_INVALID") throw error; fail(`checks[${index}].evidence.path is not locally available`); }
...
export function projectArchitectureStatus(snapshot) {
  const checks = snapshot.checks.map((check) => ({ ...check, incomplete: check.execution.state !== "completed" || check.result.state === "unknown" || check.evidence.state !== "present" || check.freshness.state !== "current" }));
  const counts = { current: checks.filter((check) => check.freshness.state === "current").length, stale: checks.filter((check) => check.freshness.state === "stale").length, unknown: checks.filter((check) => check.freshness.state === "unknown" || check.result.state === "unknown").length, missing: checks.filter((check) => check.evidence.state === "missing").length, failed: checks.filter((check) => check.execution.state === "failed").length, notRun: checks.filter((check) => check.execution.state === "not-run").length };
