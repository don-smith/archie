...
function npmProjectionV2(record: ReleaseRecordV2): NpmProjection {
  ...
  const packages: Record<string, unknown> = { "": { name: "archie-private-runtime", version: record.version, dependencies } };
  for (const artifact of record.artifacts) {
    const artifactDependencies = { ...artifact.dependencies };
    for (const local of record.artifacts) if (local.package in artifactDependencies) artifactDependencies[local.package] = local.locator;
...
function validateNpmProjectionV2(projection: NpmProjection, record: ReleaseRecordV2): void {
  ...
  for (const artifact of record.artifacts) {
    ...
    for (const local of record.artifacts) if (local.package in expectedDependencies) expectedDependencies[local.package] = local.locator;
    if (!installed || installed.version !== artifact.version || installed.resolved !== artifact.locator || installed.integrity !== artifact.lockIntegrity || JSON.stringify(installed.dependencies) !== JSON.stringify(expectedDependencies) || JSON.stringify(installed.engines) !== JSON.stringify(artifact.engines) || JSON.stringify(installed.bin) !== JSON.stringify(artifact.binaries)) throw new Error("generated npm projection differs from the pinned release record");
  }
}
