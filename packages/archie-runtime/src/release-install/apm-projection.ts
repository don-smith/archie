...
const includes = (lines: string[], value: string): boolean => lines.some(line => line.trim() === `- ${value}`);
...
function appendSection(lines: string[], name: string, content: string[]): string[] {
...
const nestedLockDependency = (record: ReleaseRecordV1): string[] => [
...
const apm029LockDependency = (record: ReleaseRecordV1): string[] => [
...
const manifestDeployment = (record: ReleaseRecordV1): string[] => ["  - target: agent-skills", `    value: .agents/skills/${scalar(record.apm.skill, "APM skill")}`];
const nestedLockDeployment = (record: ReleaseRecordV1): string[] => ["  - target: agent-skills", `    value: .agents/skills/${scalar(record.apm.skill, "APM skill")}`];
const apm029LockDeployment = (record: ReleaseRecordV1): string[] => ["- kind: project-relative", "  target: agent-skills", `  value: .agents/skills/${scalar(record.apm.skill, "APM skill")}`];
const ownsDependency = (record: ReleaseRecordV1, entry: string[]) => field(entry, "git") === record.apm.locator && includes(entry, record.apm.skill);
const ownsNestedLock = (record: ReleaseRecordV1, entry: string[]) => field(entry, "locator") === record.apm.locator && field(entry, "skill") === record.apm.skill;
const ownsApm029Lock = (record: ReleaseRecordV1, entry: string[]) => field(entry, "name") === record.apm.package && includes(entry, record.apm.skill);
const ownsDeployment = (record: ReleaseRecordV1, entry: string[]) => field(entry, "target") === "agent-skills" && field(entry, "value") === `.agents/skills/${record.apm.skill}`;

function mergeManifest(current: string | undefined, record: ReleaseRecordV1, previous?: ReleaseRecordV1): string {
  if (!current?.trim()) return ["dependencies:", "  apm:", ...dependency(record), "deployments:", ...manifestDeployment(record), ""].join("\n");
  ...
  if (!lines.includes("deployments:")) return `${appendSection(lines, "deployments", manifestDeployment(record)).join("\n")}\n`;
...
function mergeLock(current: string | undefined, record: ReleaseRecordV1, previous?: ReleaseRecordV1): string {
  if (!current?.trim()) return ["dependencies:", "  apm:", ...nestedLockDependency(record), "deployments:", ...nestedLockDeployment(record), ""].join("\n");
  let lines = current.replace(/\r\n/g, "\n").replace(/\n$/, "").split("\n");
  ...
  if (!lines.includes("deployments:")) return `${appendSection(lines, "deployments", isApm029 ? apm029LockDeployment(record) : nestedLockDeployment(record)).join("\n")}\n`;
...
export function assertPinnedApmProjection(record: ReleaseRecordV1, projection: ApmProjection): void {
  ...
  if (field(manifestDependency, "ref") !== record.apm.ref || !manifestDeployment.length) throw new Error("pinned Archie APM manifest has drifted");
  ...
  if (field(dependencyEntry, isApm029 ? "resolved_ref" : "ref") !== record.apm.ref || field(dependencyEntry, "resolved_commit") !== record.apm.resolvedCommit || field(dependencyEntry, "content_hash") !== record.apm.contentHash) throw new Error("pinned Archie APM lock has drifted");
}
