...
import { renderArchitectureStatus } from "./render-architecture-status.mjs";
...
export async function buildArchitectureDocs(configPath, { handoffOnly = false } = {}) {
  ...
  try {
    ...
    const generatedFiles = ["index.html", ...config.pages.areas.map((page) => `${page.slug}/index.html`), ...(architectureStatus.configured ? ["architecture-status/index.html"] : []), "assets/likec4-views.js", "assets/views.json", "assets/preview.json"];
    const previewMetadata = {
      ...
      build: { compiledViewCount: compiled.views.length, pageCount: pages.length, initialView: config.model.initialView, provisionalClaimCount: statuses.filter((claim) => !claim.approved).length, generatedFiles, ...(architectureStatus.configured ? { architectureStatus: true } : {}) },
    };
    if (!handoffOnly) {
      ...
      if (architectureStatus.configured) await mkdir(path.join(publicationStage, "architecture-status"), { recursive: true });
      await Promise.all([
        ...
        ...stagedPages.map(({ page, html }) => writeFile(page.id === config.pages.home.id ? path.join(publicationStage, "index.html") : path.join(publicationStage, page.slug, "index.html"), renderSite({ config, page, pageHtml: html, views: compiled.views, paletteCss: palette.css, claimStatuses: statuses, assetPrefix: page.id === config.pages.home.id ? "" : "../", statusConfigured: architectureStatus.configured }))),
        ...(architectureStatus.configured ? [writeFile(path.join(publicationStage, "architecture-status", "index.html"), renderArchitectureStatus({ config, status: architectureStatus.projection, paletteCss: palette.css }))] : []),
      ]);
    }
    handoffStage = await createPublicationStage(handoffDirectory);
    await writeHandoffBundle({ destination: handoffStage, config, compiled, pages, statuses, pagePaths: config.paths.pagePaths, archieDocumentationActive, previous, architectureStatus });
    if (!handoffOnly) {
