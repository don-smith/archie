---
template_version: 1
date: {UTC timestamp from the Metadata commands}
...
last_updated_by: {git user.name, or unknown}
...
{N} phases, ordered top-to-bottom. Each phase can become one work item in the repository's tracker or planning flow; sizing is by file count, finding count, blast-radius mix, and coordination need — never human-days.
...
**Coordination:** {none | sibling-package change required | downstream consumer release required}.
...
Plan ready to become work items.
