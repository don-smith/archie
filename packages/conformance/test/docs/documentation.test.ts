...
test("documents the supported compiler pin, CLI, and exit codes", async () => {
  ...
  assert.match(readme, /\.archie\/runtime\/node_modules\/\.bin\/architecture-conformance/); assert.doesNotMatch(readme, /npm install --save-dev/); assert.match(readme, /onboarding-state\/v1/); assert.match(readme, /onboarding-summary/); assert.match(readme, /--exclude/); assert.match(readme, /string-literal dynamic imports/);
