...
import type { GraphNode, NormalizedGraphV1 } from "../formats/types.js";

function sourceName(node: GraphNode | undefined, fallback: string): string { return node?.module ?? fallback; }
function lines(items: string[]): string { return items.length === 0 ? "- None\n" : `${items.map((item) => `- ${item}`).join("\n")}\n`; }

/** Produces a deterministic, evidence-only reading view of one normalized graph. */
export function buildOnboardingSummary(graph: NormalizedGraphV1): string {
  if (!graph.scope) throw new TypeError("normalized graph is missing selected scope");
  const byId = new Map(graph.nodes.map((node) => [node.id, node]));
  const sourceModules = graph.nodes.filter((node) => node.kind === "source-module").sort((a, b) => (a.file ?? "").localeCompare(b.file ?? ""));
  const resolved = graph.edges.filter((edge) => edge.status === "resolved"); const unresolved = graph.edges.length - resolved.length;
  const gaps = new Map<string, typeof graph.gaps>();
  for (const gap of graph.gaps) gaps.set(gap.kind, [...(gaps.get(gap.kind) ?? []), gap]);
