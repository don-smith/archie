...
const docsPackage = path.resolve("packages/capabilities/assets/architecture-docs");
const sourceDependencies = path.resolve("/Users/don/projects/c4archviewer/node_modules");
...
// The imported consumer is intentionally dependency-free at the product boundary. Its
// compiler dependencies are linked only for this integration test from the reviewed
// source checkout, rather than being added as an Archie/runtime dependency.
test.before(async () => {
  dependencyDirectory = path.join(docsPackage, "node_modules");
  await mkdir(dependencyDirectory, { recursive: true });
  for (const dependency of ["likec4", "marked"]) {
    await symlink(path.join(sourceDependencies, dependency), path.join(dependencyDirectory, dependency), "junction");
  }
...
test.after(async () => {
  if (dependencyDirectory) await rm(dependencyDirectory, { recursive: true, force: true });
});
