...
const skillSubset = (lines: string[]): string[] => lines.filter(line => /^\s*-\s+\S+\s*$/.test(line)).map(line => line.trim().slice(2)).filter(value => (ARCHIE_SKILLS as readonly string[]).includes(value)).sort();
const sameSkills = (lines: string[], skills: readonly ArchieSkill[]) => JSON.stringify(skillSubset(lines)) === JSON.stringify(skills);
...
const ownsDependency = (record: ReleaseRecordV1, entry: string[]) => field(entry, "git") === record.apm.locator && sameSkills(entry, record.apm.skills);
...
export function assertPinnedApmProjection(record: ReleaseRecordV1, projection: Required<ApmProjection>): void {
  ...
  if (field(manifestDependency, "ref") !== record.apm.ref || !sameSkills(manifestDependency, record.apm.skills)) throw new Error("pinned Archie APM manifest has drifted");
  ...
  if (field(entry, "resolved_ref") !== record.apm.ref || field(entry, "resolved_commit") !== record.apm.resolvedCommit || field(entry, "content_hash") !== record.apm.contentHash || !sameSkills(entry, record.apm.skills)) throw new Error("pinned Archie APM lock has drifted");
}
