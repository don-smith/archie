...
export function assertInstalledNpm(pin: PinnedTarget): void {
  ...
  for (const artifact of pin.record.artifacts) {
    ...
    let manifest: { name?: unknown; version?: unknown; dependencies?: unknown; engines?: unknown; bin?: unknown };
    ...
    const fullManifest = manifest as Record<string, unknown>;
    if (canonicalize(fullManifest.dependencies ?? {}) !== canonicalize(artifact.dependencies) || canonicalize(fullManifest.engines ?? {}) !== canonicalize(artifact.engines) || canonicalize(fullManifest.bin ?? {}) !== canonicalize(artifact.binaries)) throw new Error(`installed npm package metadata differs from the pinned record: ${artifact.package}`);
