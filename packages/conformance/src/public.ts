...
import { exactKeys, fail, oneOf, record, string, strings } from "./formats/helpers.js";
function paths(value: unknown, label: string): ConformanceStateReferenceV1["paths"] {
  const raw = record(value, label); exactKeys(raw, ["state", "graph", "summary", "map", "contract", "report", "baseline"], label);
...
function evidence(value: unknown, label: string): ConformanceStateReferenceV1["evidence"] {
  const raw = record(value, label); exactKeys(raw, ["graph", "summary", "map", "contract", "report", "baseline"], label);
...
function freshness(value: unknown, label: string): ConformanceFreshness {
  const raw = record(value, label); exactKeys(raw, ["status", "inputs", "reason"], label);
...
function checkpoint(value: unknown, label: string): OnboardingStateV1["checkpoint"] { return oneOf(value, ["scope-selected", "evidence-generated", "classification-drafted", "proposed-contract-checked", "active-contract-checked", "baseline-created"] as const, label); }
function stateReference(value: unknown, label: string): ConformanceStateReferenceV1 {
  const raw = record(value, label); exactKeys(raw, ["version", "identity", "checkpoint", "paths", "evidence"], label);
...
export function readConformanceReportContract(value: unknown): ConformanceReportContractV1 {
  const raw = record(value, "conformance report contract"); exactKeys(raw, ["version", "report", "state"], "conformance report contract");
  if (raw.version !== CONFORMANCE_REPORT_CONTRACT_V1) fail(`unsupported document version: ${String(raw.version)}`);
  const reportRaw = record(raw.report, "report"); exactKeys(reportRaw, ["version", "identity", "result", "evidence", "freshness"], "report");
  if (reportRaw.version !== CONFORMANCE_REPORT_V1) fail(`unsupported report version: ${String(reportRaw.version)}`);
  const identity = record(reportRaw.identity, "report.identity"); exactKeys(identity, ["id", "producer", "outputDigest"], "report.identity");
  if (identity.producer !== "@archie/conformance") fail("report.identity.producer must be @archie/conformance");
  const result = record(reportRaw.result, "report.result"); exactKeys(result, ["code", "meaning", "status"], "report.result");
  ...
  const parsedResult = { code: code as ConformanceResultCode, meaning: result.meaning as ConformanceResultMeaning, status: oneOf(result.status, codeStatuses, "report.result.status") };
  const evidenceRaw = record(reportRaw.evidence, "report.evidence"); exactKeys(evidenceRaw, ["realizationMap", "contract", "graph", "provenance"], "report.evidence");
...
export function readConformanceStateContract(value: unknown): ConformanceStateContractV1 {
  const raw = record(value, "conformance state contract"); exactKeys(raw, ["version", "stateVersion", "identity", "checkpoint", "paths", "evidence", "freshness"], "conformance state contract");
