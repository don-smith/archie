...
export function verifyOnboardingRuntime(nodeVersion = process.versions.node) {
    if (!/^24\.\d+\.\d+$/.test(nodeVersion))
        throw new Error("architecture-conformance requires Node >=24 <25; switch to the Node 24 runtime required by the pinned Archie release");
}
...
export function verifyLocalOnboardingSetup(repositoryRoot = process.cwd()) {
    ...
    if (typeof locator !== "string" || !/^file:npm\/[^/]+\.tgz$/.test(locator))
        throw new Error("Archie runtime must pin @archie/conformance to a target-owned local tarball");
    ...
    if (!inspectedTarball.files.has(`package/${normalizeNpmBinaryPath(archiveBinary)}`))
        throw new Error("target-owned tarball omits its declared architecture-conformance binary");
    const lockfile = packageJson(lockfilePath);
    const packages = record(lockfile.packages);
    const lockedRoot = record(packages?.[""]);
    const lockedDependencies = record(lockedRoot?.dependencies);
    const lockedPackage = record(packages?.["node_modules/@archie/conformance"]);
    const lockedBin = record(lockedPackage?.bin);
    ...
    if (realpathSync(resolve(dirname(installedManifestPath), installedBinary)) !== resolvedExecutable)
        throw new Error("project-local architecture-conformance binary differs from the installed package manifest");
}
