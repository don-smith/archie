...
export function assertInstalledNpm(pin) {
    ...
    for (const artifact of artifacts) {
        ...
        if (pin.record.schemaVersion === 2) {
            const fullManifest = JSON.parse(readFileSync(packagePath, "utf8"));
            const v2Artifact = artifact;
            if (canonicalize(fullManifest.dependencies ?? {}) !== canonicalize(v2Artifact.dependencies) || canonicalize(fullManifest.engines ?? {}) !== canonicalize(v2Artifact.engines) || canonicalize(fullManifest.bin ?? {}) !== canonicalize(v2Artifact.binaries))
