...
Last updated 2026-09-17. `main` is at `566b81d`; product version `0.1.0-private.1`; the eight-skill APM context is published at `git@github.com:don-smith/archie.git` tag `v0.1.0-private.1` (commit `858fd77`).
...
| ID | Item | Kind |
|---|---|---|
| N0 | **Make GitHub `don-smith/archie` the full monorepo and settle how Archie installs into a repository.** Today the monorepo has no Git remote (its source exists only on this machine) and `don-smith/archie` holds only the published APM context with its own two-commit history. Developer direction (2026-09-18): the GitHub repository should be exactly this monorepo; Archie is repository-scoped (never global) and is invoked as the first skill in a coding-agent conversation; any delivery mechanism is acceptable if installing into a repository is easy and agents find the skills under `.agents/skills/` (APM, an install script, or a skills CLI). Findings so far: APM 0.29 installs "virtual" packages from a repository subfolder (`apm install owner/repo/path#tag`; the lock records `virtual_path` and `is_virtual: true`), verified with a single-skill folder. Not yet verified: a subfolder holding the whole eight-skill APM package (`packages/archie-context`), and how release record v3's APM evidence (which expects a root package named `archie-context`) would record it. Suggested order: (1) push the monorepo to `don-smith/archie` as `main`, keeping the existing `v0.1.0-private.*` context tags so current pins still resolve; (2) test `apm install don-smith/archie/packages/archie-context#<tag>`; (3) choose the install mechanism and adapt the release record and docs. Step 1 replaces the remote `main` history, so it needs explicit approval. | decision and research (developer) |
