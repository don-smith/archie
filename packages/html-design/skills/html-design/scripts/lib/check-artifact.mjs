// Generated by scripts/bundle-html-design-checker.mjs from packages/html-design/src/check-artifact.mjs. Do not edit.
// Bundles parse5 and entities; see ../../THIRD_PARTY_NOTICES.md.

// packages/html-design/src/check-artifact.mjs
...
var URL_ATTRIBUTES = /* @__PURE__ */ new Set(["action", "data", "href", "poster", "src", "srcset"]);
var FORBIDDEN_VOCABULARY = /\bReference Project(?:\s*2\.0)?\b|\bReferenceStore\b|reference-project-/i;
var REQUIRED_ASSET_RULES = /* @__PURE__ */ new Set(["theme-asset", "palette-asset", "canonical-asset"]);
var SOURCE_PALETTE_ANCHORS = /* @__PURE__ */ new Set([
  "#111816",
  "#223d31",
  "#35413b",
  "#607068",
...
function textOf(node) {
  return (node.childNodes ?? []).map((child) => child.nodeName === "#text" ? child.value : textOf(child)).join("");
}
  ...
  for (const node of nodes) {
    ...
    if (node.tagName === "style") {
      sources.push({
        css: textOf(node),
        node,
        approved: Object.hasOwn(attrs, "data-ds-canonical") || Object.hasOwn(attrs, "data-ds-palette")
      });
    }
...
function formatArtifactReport(report) {
  ...
  return [...failures, ...suppressions, report.ok ? `Artifact is valid: ${report.path}` : ""].filter(Boolean).join("\n");
}
async function checkArtifact({ root = process.cwd(), file, profile } = {}) {
  ...
  const diagnostics = [];
  const suppressions = [];
  const seenSuppressions = /* @__PURE__ */ new Set();
  ...
  for (const node of nodes) {
    ...
    for (const [name, value] of Object.entries(attrs)) {
      if (URL_ATTRIBUTES.has(name) && (EXTERNAL_URL.test(value) || /(?:^|\s)https?:\/\//i.test(value))) {
        add(
          "external-request",
          node,
  ...
  if (!["system", "light", "dark"].includes(theme) || !schemes.includes("light") || !schemes.includes("dark")) {
    add(
      "theme-metadata",
      htmlNode,
  ...
  } else if (normalizedAsset(textOf(themeScripts[0]), { allowTrailingCommas: true }) !== normalizedAsset(currentTheme, { allowTrailingCommas: true })) {
    add(
      "theme-asset",
      themeScripts[0],
  ...
  const canonicalTokens = /* @__PURE__ */ new Set();
...
${paletteSource}`.matchAll(/(--ds-[\w-]+)\s*:/g)) {
  ...
  const sourcePalette = /* @__PURE__ */ new Set([
  ...
  for (const { css, node, approved } of cssSources) {
    if (/@import\s+(?:url\()?\s*["']?(?:https?:)?\/\//i.test(css) || /url\(\s*["']?(?:https?:)?\/\//i.test(css)) {
      add(
        "external-request",
        node,
  ...
  if (profile) {
    ...
    if (profileRecord.print !== "not required unless the application has an explicit reporting task" && !/@media\s+print/i.test(css)) {
      add(
        "profile-print",
        htmlNode,
