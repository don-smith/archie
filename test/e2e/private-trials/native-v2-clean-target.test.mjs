...
function artifact(tarball, name, locator) {
  ...
  return {
    ...
    recordArtifact: {
      ...
      requiredPlatformPayload: name === "@archie/runtime" ? "vendor/html-design/SKILL.md" : "dist/cli.js",
      dependencies: packageManifest.dependencies ?? {},
      engines: packageManifest.engines ?? {},
...
function releaseLock(runtimeArtifact, conformanceArtifact) {
  return npmProjection({ schemaVersion: 2, version: runtimeArtifact.packageManifest.version, artifacts: [runtimeArtifact.recordArtifact, conformanceArtifact.recordArtifact] }).lock;
}
...
test("v2 bootstrap installs both real packed artifacts with the exact offline projection", () => {
  const base = mkdtempSync(join(tmpdir(), "archie-native-v2-clean-target-"));
  try {
    ...
    writeFileSync(join(bundle, "bundle.json"), `${JSON.stringify({
      ...
      artifacts: [
        { package: "@archie/runtime", version: runtime.packageManifest.version, locator: runtime.lockEntry.resolved, lockFile: "npm/archie-runtime.lock.json", tarball: "npm/archie-runtime.tgz", requiredPlatformPayload: "vendor/html-design/SKILL.md" },
    ...
    assert.equal(installed.record.schemaVersion, 2);
    assert.deepEqual(npmCalls, [["ci", "--ignore-scripts", "--offline"]]);
    assert.equal(readFileSync(join(target, ".archie/runtime/package-lock.json"), "utf8"), lock, "bootstrap must use the exact finalized v2 lock");
