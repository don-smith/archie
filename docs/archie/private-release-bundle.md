...
Run `npm run private-trial:evaluate` to regenerate the retained hermetic evaluation packet at `evaluation/private-trials/latest.json`. It records deterministic finalization, replay byte manifests, drift rejection, recovery, capability authority checks, and the coordinated record-plus-artifact replacement boundary. Signing, public-release trust, controller distribution, trust roots, and key operations remain deferred.
