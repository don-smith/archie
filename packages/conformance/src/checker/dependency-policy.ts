...
export function dependencyPolicyResults(rule: DependencyPolicyRule, edges: Array<{ edge: GraphEdge; source: string; target: string }>): ReportResult[] {
  return edges.filter(({ edge, source, target }) => {
    if (!rule.sources.includes(source) || !rule.edgeKinds.includes(edge.kind)) return false;
    if (rule.world === "closed") return !rule.allowedTargets?.includes(target);
