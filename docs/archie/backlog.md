...
| N0d | **The consuming project's APM targets must include `agent-skills`, or deployment silently skips.** The context declares `targets: [agent-skills]`; installing it into a project whose targets are, say, `[claude]` logs `Package targets [agent-skills] do not overlap authorized active targets; skipping` and deploys nothing, while still exiting 0 and writing a lock. `install.sh` has to set or verify the target and fail loudly, because the failure otherwise looks like a successful install with no skills. | implementation |
| N0e | **Update the bundle fixture and `docs/archie/private-release-bundle.md` for the subfolder pin.** Both still describe the pre-monorepo shape: a root-package APM dependency and a `v<version>` tag ref. Two facts found while building N0a(a): the monorepo has no root `apm.yml`, so only the subfolder form resolves; and the plain string spec `don-smith/archie/packages/archie-context#<ref>` that `apm install` writes produces a lock with **no `skill_subset`**, which `assertPinnedApmProjection` requires — only the mapping form with a `path:` key keeps it. The generated manifest therefore must use the mapping form. | implementation |
